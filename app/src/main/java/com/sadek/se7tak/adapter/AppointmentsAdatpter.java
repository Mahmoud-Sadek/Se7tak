...
public class AppointmentsAdatpter extends RecyclerView.Adapter<AppointmentsAdatpter.ViewHolder> {
    ...
    public void onBindViewHolder(@NonNull final ViewHolder holder, @SuppressLint("RecyclerView") final int position) {
        ...
        if (data.get(position).getStuatus().equals(Common.ORDER_STATUS_ACCEPTED) || data.get(position).getStuatus().equals(Common.ORDER_STATUS_REJECTED)) {
            if (!data.get(position).isSeen()) {
                holder.notification_numbers.setVisibility(View.VISIBLE);
                holder.itemView.setOnClickListener(new View.OnClickListener() {
                    @Override
                    public void onClick(View v) {
                        data.get(position).setSeen(true);
                        fireDatabase.seenOrder(data.get(position), new FireDatabase.ResultCallback() {
                            @Override
                ...
                holder.notification_numbers.setVisibility(View.GONE);
            }
        }else{
                holder.notification_numbers.setVisibility(View.GONE);
            }
